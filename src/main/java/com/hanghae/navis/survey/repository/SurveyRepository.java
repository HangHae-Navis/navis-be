...
public interface SurveyRepository extends JpaRepository<Survey, Long> {
    @Query(value = "SELECT bb.id AS boardId, bb.title AS title, " +
            ...
            "            GROUP_CONCAT(DISTINCT so.option SEPARATOR ', ') AS options, " +
            "            GROUP_CONCAT(IFNULL( " +
            "            (SELECT " +
            "            IF(sq.type = 'DESCRIPTIVE', GROUP_CONCAT(DISTINCT a.answer SEPARATOR '|| '), COUNT(*)) " +
            "            FROM answer a " +
            "            WHERE a.survey_question_id = sq.id AND (sq.type = 'DESCRIPTIVE' OR a.survey_option_id = so.id) " +
            "            ), 'None'" +
            "            ) ORDER BY so.id ASC SEPARATOR ', ') AS answerCount " +
