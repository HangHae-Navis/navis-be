...
public interface SubmitRepository extends JpaRepository <GroupMember, Long> {
    @Query(value = "SELECT gm.user_id as userId, u.nickname as nickName, hs.submit as submit, hs.id as subjectId, " +
            "GROUP_CONCAT(hsf.file_url SEPARATOR ', ') as fileUrl, " +
            "GROUP_CONCAT(hsf.file_name SEPARATOR ', ') as fileName, " +
            "hs.late as late, hs.created_at as createdAt, gm.group_role as role, " +
            "GROUP_CONCAT(IFNULL(f.feedback, 'None') SEPARATOR ', ') as feedback " +
            ...
            "LEFT OUTER JOIN feedback f ON f.homework_subject_id = hs.id " +
