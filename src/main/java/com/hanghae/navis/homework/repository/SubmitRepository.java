...
public interface SubmitRepository extends JpaRepository <GroupMember, Long> {
    @Query(value = "SELECT gm.user_id as userId, u.nickname as nickName, hs.submit as submit, GROUP_CONCAT(hsf.file_url SEPARATOR ', ') as fileUrl, hs.late as late, hs.created_at as createdAt, gm.group_role as role FROM group_member as gm " +
